...
import {EDIT_SIZE,GET_MASTER_DATA} from '../ApiConstants';

class SizeButton extends Component {
    constructor(props) {
    super(props);
    this.state = {value:this.props.sizeval.size,style:{styleEdit:{display:'none'},styleSpan:{display:'block'}}};
  ...
  keyPress(e){
      if(e.keyCode == 13){
         ...
         this.setState( this.state);
        this.editShelf(this.state.value,this.props.sizeval);
      }
   }
editShelf(newvalue,size){
             var x={
              id:size.size,
              name:newvalue
            }
            axios.post(EDIT_SIZE,x).then(res =>{
  ...
  render() {
    var sizeval=this.props.sizeval;
    return (
   <div>
...
export default connect(matchDispatchToProps,mapStateToProps)(SizeButton);